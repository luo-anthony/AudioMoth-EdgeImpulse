...
// EI Project edit
// ANTHONY further reduced number of buffers to 2
...
#define EI_SRAM_START_ADDRESS (AM_EXTERNAL_SRAM_START_ADDRESS + (NUMBER_OF_BYTES_IN_SAMPLE * NUMBER_OF_SAMPLES_IN_BUFFER * NUMBER_OF_BUFFERS))
...
static bool readConfigurationFromFile()
{
    ...
    while (count < CONFIG_MAX_FILE_READ_CHARACTERS)
    {
        ...
        if (status == CP_SUCCESS || status == CP_CHARACTER_ERROR)
            break;
    ...
    *poweredDownWithShortWaitInterval = false;
    ...
    if (timeUntilPreparationStart <= 0)
    {
        ...
        if (writtenDeviceDataToFileInThisSession == false && *writtenDeviceDataToFile == false)
        {
            ...
            if (fileSystemEnabled)
                *writtenDeviceDataToFile = writeDeviceDataToFile(firmwareDescription, firmwareVersion, (uint8_t *)AM_UNIQUE_ID_START_ADDRESS);
        }
...
static AM_recordingState_t makeRecording(uint32_t timeOfNextRecording, uint32_t recordDuration, bool enableLED, AM_extendedBatteryState_t extendedBatteryState, int32_t temperature, uint32_t *fileOpenTime, uint32_t *fileOpenMilliseconds)
{
    ...
    AM_recordingState_t recordingState = microphoneChanged ? MICROPHONE_CHANGED : switchPositionChanged ? SWITCH_CHANGED
    ...
    setHeaderComment(&wavHeader, configSettings, timeOfNextRecording + timeOffset, (uint8_t *)AM_UNIQUE_ID_START_ADDRESS, extendedBatteryState, temperature, externalMicrophone, recordingState);
